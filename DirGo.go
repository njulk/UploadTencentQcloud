...
func (cos *COS) ListDir(configurename string, n *sync.WaitGroup, dirpath string, selectSubdir bool, dirschan chan string, errRets chan error) {
	...
	if errRet != nil {
		...
		errRets <- errRet
		cos.log.Error("配置文件%s:读取目录%s出现错误:%s", configurename, dirpath, errRet.Error())
		return
	}
	for _, fi := range dir {
		if fi.IsDir() {
			dirs = append(dirs, filepath.Join(dirpath, fi.Name())) //此处将多余的斜杠去除，将缺少的斜杠添加
		} else {
			files = append(files, filepath.Join(dirpath, fi.Name()))
			//fmt.Println(filepath.Join(dirpath, fi.Name()))
...
func (cos *COS) getDir(configurename, dirpath string, selectSubdir bool) (files []string, errRet error) {
	...
	for {
		...
		case _, ok := <-errRets:
			...
			cos.outlog.Error("配置文件%s:%s", configurename, errRet.Error())
			break
		}
...
func (cos *COS) extractDir(configurename string, localFile string) (filename string, errRet error) {
	if localFile == "" {
		...
		cos.log.Error("%s:提取文件名出错，文件路径名空", configurename)
...
func (cos *COS) uploadAllfiles(configurename string, allFiles []string) (errRet error) {
	...
	if errRet != nil {
		...
		cos.log.Error("%s:%s", configurename, errRet.Error())
		return
	}
	num := (runtime.NumCPU()*2 + 1) / configSize
	if num == 0 {
		num = 3
	}
	if len(allFiles) > num {
	...
	if errRet != nil {
		cos.log.Error("%s:分析记录文件在cos的路径时出错", configurename)
		return
	}
...
func (cos *COS) uploadFromlocal(configurename string, filedir string, selectSubdir bool) (errRet error) {
	...
	if len(files) == 0 {
		cos.log.Warn("配置文件:%s匹配文件个数为0,或许路径下忘记添加*", configurename)
		cos.outlog.Warn("配置文件%s:匹配到文件个数为0,或许路径下忘记添加*", configurename)
		return
	}
